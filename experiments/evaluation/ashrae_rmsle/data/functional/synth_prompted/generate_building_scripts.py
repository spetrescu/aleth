...
Make the measurements resemble real-world data as closely as possible, ensuring the dataset appears realistic.
...
def check_ollama_server() -> None:
    ...
    if not models:
        print("Ollama is reachable, but no models were listed.")
    else:
        print(f"Ollama reachable. Models: {', '.join(models)}")

    if OLLAMA_MODEL not in models:
        print(
            f"Requested model '{OLLAMA_MODEL}' was not found. "
...
def extract_code(text: str) -> str:
    ...
    if begin_index != -1:
        ...
        if code:
            print("Missing <CODE END>; saving partial code anyway.")
            return code
    ...
    if "def " in text or "import " in text or "if __name__ ==" in text:
        print("[WARN] No tokens found; saving raw model output as code.")
        return text
...
def main() -> int:
    ...
    with BUILDINGS_CSV.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            print(f"Generating code for building {building_id}...")
