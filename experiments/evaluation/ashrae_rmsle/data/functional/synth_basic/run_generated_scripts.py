...
def main() -> int:
    ...
    for py_file in py_files:
        print(f"[INFO] Running {py_file.name}...")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"{py_file.name} failed with code {result.returncode}", file=sys.stderr)
                if result.stderr.strip():
                    print(result.stderr.strip(), file=sys.stderr)
                failed += 1

        except Exception as exc:
            print(f"[ERROR] Exception while running {py_file.name}: {exc}", file=sys.stderr)
            failed += 1
